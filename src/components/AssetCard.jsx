  ...
  const handleSend = async (e) => {
    ...
    try {
      if (coin.symbol === "ETH" || coin.symbol === "BTC") {
        ...
        const signature = await blockchain.sendSol(
          coin.privateKey,
          recipient,
          parseFloat(amount),
        );
  ...
  return (
    <div className="card">
      <div className="card-body">
        ...
        <p className="balance-display my-2">
          {balance === null ? (
        ...
        {message && (
          <div className="alert alert-success mt-4 p-3">{message}</div>
        )}
        {error && <div className="alert alert-error mt-4 p-3">{error}</div>}
      </div>
    </div>
  );
}
