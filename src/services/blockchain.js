...
export async function getBtcBalance(address) {
  try {
    ...
    if (!response.ok)
      throw new Error(`BTC balance fetch failed for address: ${address}`);
...
export async function sendBtc(privateKeyWIF, toAddress, amountBTC) {
  ...
  const utxoResponse = await fetch(
    `https://blockchain.info/unspent?active=${fromAddress}`,
  );
  const utxoData = await utxoResponse.json();
  const utxos = utxoData.unspent_outputs;
  if (utxos.length === 0)
    throw new Error("No spendable outputs (UTXOs) found.");
  ...
  for (const utxo of utxos) {
    ...
    const txHex = await (
      await fetch(
        `https://blockchain.info/rawtx/${utxo.tx_hash_big_endian}?format=hex`,
      )
    ).text();
    psbt.addInput({
